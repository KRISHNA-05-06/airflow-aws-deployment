...
# Task 1: Generate mock event data
def generate_fake_events():
    events = [
        ...
        "New particle at CERN", "NASA Moon base plan", "Tremors in Tokyo",
        "Open-source boom"
    ]
...
#Task 3: Upload to S3 (configured in Part II)
def upload_to_s3(**kwargs):
    run_date = kwargs['ds']
    bucket_name = 'srikrishna-airflow-bucket'          # ← will be updated in Part II
    s3_key = f'events/events_transformed_{run_date}.csv'
    s3 = boto3.client('s3')
    s3.upload_file(transformed_path, bucket_name, s3_key)
    print(f"[S3] Uploaded to s3://{bucket_name}/{s3_key}")
